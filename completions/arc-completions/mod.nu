...
module arc-completion-utils {
  export const ARC_SKIPABLE_FLAGS = ['-v', '--version', '-h', '--help', '-p', '--paginate', '-P', '--no-pager', '--no-replace-objects', '--bare']
  ...
  # Get changed files which can be restored by `arc checkout --`
  export def get-changed-files []: nothing -> list<string> {
    ^arc status -uno | lines
    | where $it =~ '^1 [.MD]{2}'
    | each { split row ' ' -n 9 | last }
  }

  # Get files which can be retrieved from a branch/commit by `arc checkout <tree-ish>`
  ...
  export def get-all-arc-local-refs []: nothing -> list<record<ref: string, obj: string, upstream: string, subject: string>> {
    ^arc for-each-ref --format '%(refname:lstrip=2)%09%(objectname:short)%09%(upstream:remotename)%(upstream:track)%09%(contents:subject)' refs/heads | lines | parse "{ref}\t{obj}\t{upstream}\t{subject}"
  }

  export def get-all-arc-remote-refs []: nothing -> list<record<ref: string, obj: string, subject: string>> {
    ^arc for-each-ref --format '%(refname:lstrip=2)%09%(objectname:short)%09%(contents:subject)' refs/remotes | lines | parse "{ref}\t{obj}\t{subject}"
  }

  # Get local branches, remote branches which can be passed to `arc merge`
  export def get-mergable-sources []: nothing -> list<record<value: string, description: string>> {
    (nu-complete arc local branches)
    | parse "{value}"
    | insert description "local branch"
    | append (nu-complete arc commits all)
  }
}

def "nu-complete arc available upstream" [] {
  ^arc branch -a | lines | each { |line| $line | str replace '* ' "" | str trim }
}

def "nu-complete arc remotes" [] {
  ^arc remote --verbose
  | parse --regex '(?<value>\S+)\s+(?<description>\S+)'
  | uniq-by value # Deduplicate where fetch and push remotes are the same
}

def "nu-complete arc log" [] {
  ^arc log --pretty=%h | lines | each { |line| $line | str trim }
}

# Yield all existing commits in descending chronological order.
def "nu-complete arc commits all" [] {
  ^arc log --oneline -n 400 | lines | parse "{value} {description}"
}

# Yield local branches like `main`, `feature/typo_fix`
def "nu-complete arc local branches" [] {
  ^arc branch | lines | each { |line| $line | str replace '* ' "" | str replace '+ ' ""  | str trim }
}

# Yield local and remote branch names which can be passed to `arc merge`
def "nu-complete arc mergable sources" [] {
  use arc-completion-utils *
...
def "nu-complete arc checkout" [context: string, position?:int] {
  use arc-completion-utils *
  let preceding = $context | str substring ..$position
  # See what user typed before, like 'arc checkout a-branch a-path'.
  ...
  let prev_tokens = $preceding | str trim | args-split | where ($it not-in $ARC_SKIPABLE_FLAGS)
  # In these scenarios, we suggest only file paths, not branch:
  # - After '--'
  # - First arg is a branch
  # If before '--' is just 'arc checkout' (or its alias), we suggest "dirty" files only (user is about to reset file).
  if $prev_tokens.2? == '--' {
  ...
  let commits = ^arc log -n 100 --oneline
    | lines
    | split column -n 2 ' ' value description
    | upsert description {|x| $'Commit, ($x.value) ($x.description)' }
    | insert style 'light_cyan_dimmed'
  {
    options: {
        case_sensitive: false,
        completion_algorithm: prefix,
...
# Arguments to `arc rebase --onto <arg1> <arg2>`
def "nu-complete arc rebase" [] {
  (nu-complete arc local branches)
  | parse "{value}"
  | insert description "local branch"
  | append (nu-complete arc commits all)
...
def "nu-complete arc stash-list" [] {
  arc stash list | lines | parse "{value}: {description}"
}

def "nu-complete arc tags" [] {
  ^arc tag --no-color | lines
}

# See `man arc-status` under "Short Format"
...
def "nu-complete arc built-in-refs" [] {
  [HEAD FETCH_HEAD ORIG_HEAD]
}

def "nu-complete arc refs" [] {
  nu-complete arc local branches
  | parse "{value}"
  | insert description Branch
  | append (nu-complete arc tags | parse "{value}" | insert description Tag)
  | append (nu-complete arc built-in-refs)
}

def "nu-complete arc files-or-refs" [] {
  nu-complete arc local branches
  | parse "{value}"
  | insert description Branch
  | append (nu-complete arc files | where description == "Modified" | select value)
  | append (nu-complete arc tags | parse "{value}" | insert description Tag)
  | append (nu-complete arc built-in-refs)
}

def "nu-complete arc subcommands" [] {
  ^arc list-commands | from json | select command description? | rename value | where not ($it.value starts-with '--')
}

def "nu-complete arc add" [] {
  nu-complete arc files
}
...
def "nu-complete arc merge strategies" [] {
  ['ort', 'octopus']
}

def "nu-complete arc merge strategy options" [] {
  ['ours', 'theirs']
}


# Check out arc branches and files
export extern "arc checkout" [
  ...targets: string@"nu-complete arc checkout"   # name of the branch or files to checkout
  --conflict: string                              # conflict style (merge or diff3)
  --detach(-d)                                    # detach HEAD at named commit
  --force(-f)                                     # force checkout (throw away local modifications)
  --guess                                         # second guess 'arc checkout <no-such-branch>' (default)
...
export extern "arc reset" [
  ...targets: string@"nu-complete arc checkout"      # name of commit, branch, or files to reset to
...
export extern "arc fetch" [
  repository?: string@"nu-complete arc remotes" # name of the branch to fetch
  --all                                         # Fetch all remotes
  --append(-a)                                  # Append ref names and object names to .arc/FETCH_HEAD
  ...
  --update-shallow                              # Update .arc/shallow to accept new refs
  ...
  --auto-maintenance                            # Run 'arc maintenance run --auto' at the end (default)
  --no-auto-maintenance                         # Don't run 'arc maintenance' at the end
  --auto-gc                                     # Run 'arc maintenance run --auto' at the end (default)
  --no-auto-gc                                  # Don't run 'arc maintenance' at the end
  ...
  --quiet(-q)                                   # Silence internally used arc commands
...
# Push changes
export extern "arc push" [
  remote?: string@"nu-complete arc remotes",         # the name of the remote
  ...refs: string@"nu-complete arc local branches"   # the branch / refspec
  --all                                              # push all refs
  ...
  --set-upstream(-u)                                 # set upstream for arc pull/status
...
# Pull changes
export extern "arc pull" [
  remote?: string@"nu-complete arc remotes",         # the name of the remote
  ...refs: string@"nu-complete arc local branches",  # the branch / refspec
  --rebase(-r): string@"nu-complete arc pull rebase",    # rebase current branch on top of upstream after fetching
  ...
  --update-shallow                                   # update .arc/shallow with new refs
...
export extern "arc switch" [
  switch?: string@"nu-complete arc switch"        # name of branch to switch to
  --create(-c)                                    # create a new branch
  --detach(-d): string@"nu-complete arc log"      # switch to a commit in a detached state
...
export extern "arc cherry-pick" [
  commit?: string@"nu-complete arc commits all" # The commit ID to be cherry-picked
...
export extern "arc rebase" [
  branch?: string@"nu-complete arc rebase"    # name of the branch to rebase onto
  upstream?: string@"nu-complete arc rebase"  # upstream branch to compare against
  ...
  --onto?: string@"nu-complete arc rebase"    # starting point at which to create the new commits
  --root                                      # start rebase from root commit
]

# Merge from a branch
export extern "arc merge" [
  # For now, to make it simple, we only complete branches (not commits) and support single-parent case.
  branch?: string@"nu-complete arc mergable sources"         # The source branch
  --edit(-e)                                                 # Edit the commit message prior to committing
  ...
  --strategy(-s): string@"nu-complete arc merge strategies"  # Merge strategy
  -X: string@"nu-complete arc merge strategy options"        # Option for merge strategy
  --verbose(-v)
  --help
]

export extern "arc blame" [
  path: string
  -s                        # suppress author name and timestamp
  --json                    # output in json format
  -w                        # ignore whitespace when comparing lines
  --ignore-rev              # ignore changes made by the provided revisions when assigning blame
  --ignore-revs-file        # ignore changes made by revisions listed in provided file. When specifying a relative path, the path to the file is calculated relative current working directory. If optional path is not specified then
                            # .arc-blame-ignore-revs at repository root directory will be used.
  --show-pulls              # also consider merge commits that are not TREESAME to the first parent but are TREESAME to a later parent
  -L                        # annotate only the specified lines. Can be used multiple times. <start> and <end> are optional. If <start> is ommited, it will default to the first line. If <end> is ommited, it will default to the last
                            # line. Otherwise line number must be used (count starts from 1)
  --first-parent            # walk through first parent only
  --full-topo               # full topology history walk
]

# List or change branches
export extern "arc branch" [
  branch?: string@"nu-complete arc local branches"               # name of branch to operate on
  ...
  --set-upstream-to: string@"nu-complete arc available upstream" # set upstream for branch
  ...
  --list                                                         # list branches
  --contains: string@"nu-complete arc commits all"               # show only branches that contain the specified commit
...
export extern "arc remote" [
  --verbose(-v)                            # Show URL for remotes
]

# Add a new tracked repository
export extern "arc remote add" [
]

# Rename a tracked repository
export extern "arc remote rename" [
  remote: string@"nu-complete arc remotes"             # remote to rename
  new_name: string                                     # new name for remote
]

# Remove a tracked repository
export extern "arc remote remove" [
  remote: string@"nu-complete arc remotes"             # remote to remove
]

# Get the URL for a tracked repository
export extern "arc remote get-url" [
  remote: string@"nu-complete arc remotes"             # remote to get URL for
]

# Set the URL for a tracked repository
export extern "arc remote set-url" [
  remote: string@"nu-complete arc remotes"             # remote to set URL for
  url: string                                          # new URL for remote
]

# Show changes between commits, working tree etc
export extern "arc diff" [
  rev1_or_file?: string@"nu-complete arc files-or-refs"
  rev2?: string@"nu-complete arc refs"
...
export extern "arc commit" [
  ...
  --force(-f)                                         # Commit even if prohibited by config
  --skip-hook: string                                 # Skip specific pre-commit hook
  ...pathspec: string                                 # commit files matching pathspec
]

# List commits
export extern "arc log" [
...
export extern "arc reflog" [
]

# Stage files
export extern "arc add" [
  ...file: string@"nu-complete arc add"               # file to add
  --all(-A)                                           # add all files
...
# Note that stashes are kept in a stack at a special ref `stash`
export extern "arc stash" []

# Stash changes for later
export extern "arc stash push" [
  --patch(-p)                                         # interactively choose hunks to stash
]

# Unstash previously stashed changes
export extern "arc stash pop" [
  stash?: string@"nu-complete arc stash-list"          # stash to pop
  --index(-i)                                          # try to reinstate not only the working tree's changes, but also the index's ones
]

# List stashed changes
export extern "arc stash list" [
]

# Show a stashed change
export extern "arc stash show" [
  stash?: string@"nu-complete arc stash-list"
  -U                                                  # show diff
]

# Drop a stashed change
export extern "arc stash drop" [
  stash?: string@"nu-complete arc stash-list"
]

# Create a new arc repository
export extern "arc init" [
  --initial-branch(-b): string                         # initial branch name
]

# List or manipulate tags
export extern "arc tag" [
  --delete(-d): string@"nu-complete arc tags"         # delete a tag
]

# Prune all unreachable objects
export extern "arc prune" [
  --dry-run(-n)                                       # dry run
...
export extern "arc bisect bad" [
]

# Mark the current (or specified) revision as good
export extern "arc bisect good" [
]

# Use to automatically bisect
export extern "arc bisect run" [
]

# End bisection
export extern "arc bisect reset" [
]

def "nu complete arc help options" [] {[
  {value: man, description: "Man page format, the most convenient way to see the docs"},
  {value: simple, description: 'Simple text in pager like "less". Used if current system does not support man pager'},
  {value: troff, description: "Man page formatter, can be used with another tool to read man"}
]}
def "nu complete arc help subcommands" [] {[
  {value: env, description: "Print docs about environment variables"},
  {value: commands, description: "Print all arc commands in readable format (doesn't support --format)"},
  {value: config, description: "Print docs about config variables"},
]}
# Show help for a arc subcommand
export extern "arc help" [
  command?: string@"nu complete arc help subcommands" # subcommand to show help for
  --svnrevision         # print svn version
  --help(-h)            # print usage
  --format: string@"nu complete arc help options" # output format, supported: man, simple, troff
]

# Print lines matching a pattern
export extern "arc grep" [
...
export extern "arc" [
  command?: string@"nu-complete arc subcommands"   # Subcommands
  --version(-v)                                    # Prints the Arc suite version that the arc program came from
